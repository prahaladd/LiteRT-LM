...
class ExecutionManager {
 public:
  ...
  absl::Status AddDecodeTask(
      ...
      std::shared_ptr<std::atomic<bool>> absl_nonnull cancelled,
      absl::AnyInvocable<void(absl::StatusOr<Responses>)> callback) {
    return AddDecodeTask(
        session_id, task_id, std::move(dep_tasks),
        std::move(repetition_penalty_config), std::move(no_repeat_ngram_config),
        std::move(suppress_tokens_config), constraint, std::move(cancelled),
        std::move(callback), std::numeric_limits<int>::max());
  }
